...
md"""
Run the fleet for period of time (range of weeks), with given demand
	and given maintenance plan.
...
function maintenanceplan(fleet, weeks;
	...
	for iplant=1:nplants
		wx=w0
		for imaint=1:nmaint
			# look for free maintenance slot
			while !all( 
				s->s==0,
		...
		w0=w0+duration+rand(1:5)
	end
	@assert all(s->s≤1, sum(weeklyplan,dims=1))
...
# ╔═╡ 86ccfd8c-e80f-4524-9e1b-d77b3409c379
 begin
